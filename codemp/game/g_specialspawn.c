...
//------------------------------------------------------------------------------------------------------------
static void paratur1_pain( gentity_t *self, gentity_t *attacker, int damage )
//------------------------------------------------------------------------------------------------------------
{
	if (self->target_ent)
	{
...
//------------------------------------------------------------------------------------------------------------
static void paratur1_base_pain( gentity_t *self, gentity_t *attacker, int damage )
//------------------------------------------------------------------------------------------------------------
{
	if (self->target_ent)
	{
		...
		paratur1_pain(self->target_ent, attacker, damage);
	}
}

//------------------------------------------------------------------------------------------------------------
static void paratur1_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
//------------------------------------------------------------------------------------------------------------
{
...
//------------------------------------------------------------------------------------------------------------
static void paratur1_base_die ( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int meansOfDeath )
//------------------------------------------------------------------------------------------------------------
{
	if (self->target_ent && self->target_ent->health > 0)
	{
		...
		paratur1_die(self->target_ent, inflictor, attacker, damage, meansOfDeath);
	}
}
...
//----------------------------------------------------------------
static void paratur1_fire ( gentity_t *ent, vec3_t start, vec3_t dir )
//----------------------------------------------------------------
{
	vec3_t		org;
	gentity_t	*bolt;
...
//-----------------------------------------------------
static void paratur1_think( gentity_t *self )
//-----------------------------------------------------
{
	...
	if ( self->enemy && self->setTime < level.time && self->attackDebounceTime < level.time )
	{
		...
		*/
		...
		paratur1_fire( top, org, fwd );
		self->fly_sound_debounce_time = level.time;//used as lastShotTime
	}
}

//-----------------------------------------------------
static void paratur1_aim( gentity_t *self )
//-----------------------------------------------------
{
...
//-----------------------------------------------------
static void paratur1_turnoff( gentity_t *self )
//-----------------------------------------------------
{
	gentity_t *top = &g_entities[self->r.ownerNum];
	if ( top != NULL )
...
//-----------------------------------------------------
static void paratur1_sleep( gentity_t *self )
//-----------------------------------------------------
{
	if ( self->enemy == NULL )
	{
...
//-----------------------------------------------------
static qboolean paratur1_find_enemies( gentity_t *self )
//-----------------------------------------------------
{
...
//-----------------------------------------------------
static void paratur1_base_think( gentity_t *self )
//-----------------------------------------------------
{
...
//-----------------------------------------------------------------------------
static void paratur1_base_use( gentity_t *self, gentity_t *other, gentity_t *activator )
//-----------------------------------------------------------------------------
{
	// Toggle on and off
...
qboolean paratur1_spawn( gentity_t *base )
{
	vec3_t		org;

	gentity_t *top = G_Spawn();
	top->sspawn_ent = qtrue;
	if ( !top )
	{
		return qfalse;
	}
	...
	base->pain = paratur1_base_pain;
	base->die = paratur1_base_die;

	base->mass = 1100; //shotspeed
	top->mass = base->mass;
	...
	top->pain = paratur1_pain;
	top->die  = paratur1_die;
...
void G_SpawnTurret( vec3_t pos) {

	gentity_t * base = G_Spawn();
	base->sspawn_ent = qtrue;
	VectorCopy(pos, base->s.origin);
	...
	base->use = paratur1_base_use;
	base->think = paratur1_base_think;
